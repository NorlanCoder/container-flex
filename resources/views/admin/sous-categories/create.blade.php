...
<div class="container px-6 mx-auto grid">
    ...
    <div class="w-full overflow-hidden rounded-lg shadow-xs">
        <div class="w-full overflow-x-auto">
            <form action="{{ route('souscategory.store') }}" enctype="multipart/form-data" class="w-full"
                method="post">
                @csrf
                <div class="flex flex-col w-1/2 mt-4">
                    ...
                    <input type="text"
                        ...
                        placeholder="Saisir Libellé" name="label" id="label">
                </div>
                <div class="flex flex-col w-1/2 mt-4">
                    ...
                    <select name="category_id" id="category_id"
                        ...
                        @if (count($categories))
                            @foreach ($categories as $category)
                                <option class=" text-black" value="{{ $category->id }}">{{ $category->label }}</option>
                            @endforeach
                        @endif
                    </select>
                </div>
                ...
                @endif
                <div class="mt-4">
                    <button type="submit" class="bg-white text-gray-600 p-2 px-4">Envoyer</button>
                </div>
            </form>
        </div>
