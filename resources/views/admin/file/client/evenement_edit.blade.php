    ...
    <div class="container px-6 mx-auto grid">
        ...
        <div class="w-full overflow-hidden rounded-lg shadow-xs">
            <div class="w-full overflow-x-auto">
                <form action="{{ route('adminevenementeditpost') }}" enctype="multipart/form-data" class="w-full"
                    method="post">
                    @csrf
                    <div class=" flex-col w-1/2 mt-4 hidden">
                        <input type="text" value="{{ $event->id }}"
                            class="w-full border border-[#444] bg-transparent text-black dark:text-white p-2" name="id"
                            id="id">
                    </div>
                    <div class="flex flex-col w-1/2 mt-4">
                        <label for="libelle" class="text-black dark:text-white mb-2">Libellé</label>
                        <input type="text" value="{{ $event->libelle }}"
                    ...
                    <div class="flex flex-col w-1/2 mt-4">
                        ...
                        <input type="text" value="{{ $event->equipe1 }}"
                            class="w-full border border-[#444] bg-transparent text-black dark:text-white p-2"
                    ...
                    <div class="flex flex-col w-1/2 mt-4">
                        <label for="score1" class="text-black dark:text-white mb-2">Score Equipe 1</label>
                        <input type="text" value="{{ $event->score1 != ' ' ? $event->score1 : '' }}"
                            class="w-full border border-[#444] bg-transparent text-black dark:text-white p-2"
                            placeholder="Score équipe 1" name="score1" id="score1">
                    </div>
                    <div class="flex flex-col w-1/2 mt-4">
                        <label for="score2" class="text-black dark:text-white mb-2">Score Equipe 2</label>
                        <input type="text" value="{{ $event->score2 != ' ' ? $event->score2 : '' }}"
                            class="w-full border border-[#444] bg-transparent text-black dark:text-white p-2"
                            placeholder="Score équipe 2" name="score2" id="score2">
                    </div>
                    <div class="flex flex-col w-1/2 mt-4">
                        <label for="coupon" class="text-black dark:text-white mb-2">Coupon</label>
                        <select name="coupon_id" id="coupon_id"
                            class="w-full border border-[#444] bg-transparent text-black dark:text-white p-2">
                            <option value="{{ $event->coupon->id }}">{{ $event->coupon->reference }}</option>
                            @if (count($coupons))
                                @foreach ($coupons as $coupon)
                                    @if ($coupon->id != $event->coupon->id)
                    ...
                    <div class="flex flex-col w-1/2 mt-4">
                        <label for="status" class="text-black dark:text-white mb-2">Status (Accepté, En cours, Gagné,
                            Perdu)</label>
                        <input type="text" value="{{ $event->status }}"
                            class="w-full border border-[#444] bg-transparent text-black dark:text-white p-2"
                            placeholder="Saisir un status" name="status" id="status">
                    </div>
                    <div class="mt-4">
